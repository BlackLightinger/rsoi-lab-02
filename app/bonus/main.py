...
def get_privilege_by_username(
        username: str,
        db: Session = Depends(get_db)
):
    privilege = (
        db.query(PrivilegeDb)
        .filter(PrivilegeDb.username == username)
        .first()
    )
    ...
    if not privilege:
        raise HTTPException(
            status_code=404,
            detail="Privilege not found for this user"
        )
...
def get_privilege_history_by_username(
        username: str,
        db: Session = Depends(get_db)
):
    privilege = (
